...
step_backlight() {
    for backlight in /sys/class/backlight/*/; do
        ...
        case "$1" in
            +) printf '%s' "$(( brightness + step ))" >"$backlight/brightness" ;;
            -) printf '%s' "$(( brightness - step ))" >"$backlight/brightness" ;;
        esac
    done
}
...
case "$1" in
    ...
    button/lid)
        case "$3" in
            close)
                # Skip suspend while an external display is connected
                for connector in /sys/class/drm/card*-DP-*/status /sys/class/drm/card*-HDMI-*/status
                do
                    [ -e "$connector" ] || continue
                    if [ "$(cat "$connector")" = "connected" ]
                    then
                        logger "LID closed but external display connected, skipping suspend"
                        exit 0
                    fi
                done
                # suspend-to-ram
